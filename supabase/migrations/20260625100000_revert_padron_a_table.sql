-- ════════════════════════════════════════════════════════════════════════
-- REVERT: restaurar funciones de padrón con TABLE (no jsonb)
-- El tipo jsonb hacía que PostgREST envolviera la respuesta en
-- [{"fn_name": [...]}] rompiendo el cliente.
-- La paginación se maneja desde el cliente con .range().
-- ════════════════════════════════════════════════════════════════════════

-- ── 1. padron_zona_dirigente → RETURNS TABLE ─────────────────────────────────
...
-- ── 2. buscar_padron_presidente → RETURNS TABLE ──────────────────────────────
...
begin
  ...
  -- ── Sin texto: devolver todo el padrón con filtros ────────────────────────
  -- Sin LIMIT: el cliente pagina con .range() para superar el max-rows
  if length(q) = 0 then
    return query
      select
        p.id, p.codigo::text, p.nombre_completo, p.cedula,
