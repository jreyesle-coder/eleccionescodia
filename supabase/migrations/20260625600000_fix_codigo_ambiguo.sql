-- ════════════════════════════════════════════════════════════════════════
-- Fix: "column reference codigo is ambiguous" en el LEFT JOIN.
-- Solución: calificar codigo con alias de tabla dentro del subquery.
-- ════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- ── Sin texto: LEFT JOIN con alias explícito para evitar ambigüedad ───────
  if length(q) = 0 then
    return query
      select
        p.id, p.codigo::text, p.nombre_completo, p.cedula,
        p.telefono, p.celular, p.regional, p.provincia, p.nucleo, p.carrera,
        p.pensionado, p.nuevo_integrante,
        coalesce(p.monto_deuda, 0) > 0 or (dv.dv_codigo is not null) as tiene_deuda,
        coalesce(p.monto_deuda, 0) as monto_deuda,
        p.centro_votacion, p.confirmado_por, p.confirmacion_intencion, p.confirmacion_at
      from public.padron p
      left join (select distinct x.codigo as dv_codigo from public.deudas_votantes x) dv
        on dv.dv_codigo = p.codigo
