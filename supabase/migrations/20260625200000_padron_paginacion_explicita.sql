-- ════════════════════════════════════════════════════════════════════════
-- Paginación explícita en funciones de padrón
-- Agrega p_limit / p_offset a buscar_padron_presidente y
-- padron_zona_dirigente para no depender del header Range de PostgREST.
-- ════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- ── Sin texto: devolver padrón con paginación explícita ───────────────────
  if length(q) = 0 then
    return query
      select
        p.id, p.codigo::text, p.nombre_completo, p.cedula,
...
-- ── 2. padron_zona_dirigente con paginación explícita ────────────────────────
