-- ════════════════════════════════════════════════════════════════════════
-- Reemplaza EXISTS correlated subquery por LEFT JOIN en el path sin texto.
-- Solo modifica el path "sin texto" (que devuelve miles de filas).
-- Los paths de cédula, código y nombre quedan intactos.
-- ════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- ── Sin texto: LEFT JOIN para evitar subquery correlated por fila ─────────
  if length(q) = 0 then
    return query
      select
        p.id, p.codigo::text, p.nombre_completo, p.cedula,
        ...
        p.centro_votacion, p.confirmado_por, p.confirmacion_intencion, p.confirmacion_at
      from public.padron p
      left join (select distinct codigo from public.deudas_votantes) dv
        on dv.codigo = p.codigo
