  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-white px-6 py-10">
      <div className="flex flex-row items-center justify-center gap-8 w-full max-w-4xl">
        ...
        <Link
          href="/consulta"
          className="w-full text-center font-bold text-base py-4 px-6 rounded-2xl transition-all hover:scale-105 shadow"
          style={{ backgroundColor: 'var(--color-dorado)', color: '#0e1c42' }}
        >
          Verifícate para Votar
        </Link>

        {/* Botón / Form Operación de Campaña */}
        {!mostrarLogin ? (
          <button
            onClick={() => setMostrarLogin(true)}
            className="w-full text-center font-bold text-base py-4 px-6 rounded-2xl border-2 transition-all hover:scale-105"
