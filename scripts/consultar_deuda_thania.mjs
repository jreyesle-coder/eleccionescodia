/**
 * PASO 2 — Consulta la deuda REAL en codiaenlinea.com para la listado de Thania Rodríguez
 ...
 *
 * Uso (desde C:\Users\usuario\eleccionescodia):
 *   node scripts/consultar_deuda_thania.mjs
 *
...
// ── Lista de códigos ──────────────────────────────────────────────────────────
// Los 25 códigos del PDF. MANUEL MENA viene sin colegiatura: resolver su
// código con el bloque C de scripts/ubicar_colegiados_thania.sql y agregarlo.

const CODIGOS = [
  17878, 21990,  3345,  8383,  5572, 42432, 12619, 21294, 22763,
  29156, 16866, 19319, 18319, 24342, 21409, 23657, 32849, 45244,
  35851, 33981, 40529, 31966, 24555, 34670, 41394,
  // ── pendiente ──
  // MANUEL MENA (sin colegiatura en el PDF)
]
...
const encontrados = new Set(pares.map(p => Number(p.codigo)))
const faltantes = CODIGOS.filter(c => !encontrados.has(c))
if (faltantes.length > 0) {
  console.log(`⚠ Sin cédula o inexistentes en padron: ${faltantes.join(', ')}\n`)
}
...
console.log('\nSiguiente: corre scripts/reporte_thania.sql en el SQL Editor y exporta CSV.')
